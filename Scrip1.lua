...
-- Check Zone, Rarity, and Size constraints (Strict Detection Fix)
local function IsEggValid(obj)
    local name = obj.Name:lower()

    -- 1. ไม่เอาสัตว์เลี้ยง (Pet/Animal) หรือผู้เล่น
    if name:find("pet") or name:find("animal") or obj:FindFirstChildOfClass("Humanoid") then
        return false
    end

    -- 2. ต้องเป็นคำว่า egg เท่านั้น
    if not name:find("egg") then
        return false
    end

    -- 3. ตัดไข่ที่อยู่ในสวน ฐาน หรือตัวผู้เล่นออก
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChildOfClass("Humanoid") then
        ...
        if Config.Noclip then
            for     TargetRarity = "All", -- "All", "Common", "Rare", "Epic", "Legendary", "Mythic"
